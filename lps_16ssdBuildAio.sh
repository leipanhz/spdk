...
# Ensure kernel target is OFF
sudo modprobe -r nvmet_rdma nvmet
...
# Create 16 aio block devices
for i in {0..15}; do
  # sudo ./scripts/rpc.py bdev_aio_create /dev/nvme${i}n1 aio${i} 4096; sleep 2;
  sudo ./scripts/rpc.py bdev_aio_create /dev/nvme${i}n1 aio${i} 512;
done

# Configure NVMe-oF Transport with RDMA
